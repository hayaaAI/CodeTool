<template>
    <div style="width: 680px;margin-left: 15%">
  ...
  import webstore from '@/webstore'
    ...
    export default {
        ...
        methods: {
            ...
            get: function(id) {
                var that = this;
                httphelper.authedpostform(webstore.urls.codeTemplateGetUrl, {"id": id},
                    function (data) {
                        that.ruleForm = data;
                    });
            },
                ...
                this.$refs[formName].validate(function(valid) {
                    if (valid) {
                        if (that.ruleForm.codeTemplateId == 0) {
                            httphelper.authedpostform(webstore.urls.codeTemplateAddUrl,{"info":that.ruleForm,"solutionTemplateId":that.solutionTemplateId},
                                function (data) {
                                    that.back();
                                });
                        } else {
                            httphelper.authedpostform(webstore.urls.codeTemplateEditUrl, {"info":that.ruleForm,"solutionTemplateId":that.solutionTemplateId},
                                function (data) {
                                    if (data)
