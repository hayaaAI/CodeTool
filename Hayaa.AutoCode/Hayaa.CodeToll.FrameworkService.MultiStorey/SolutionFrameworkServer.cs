...
namespace Hayaa.CodeTool.FrameworkService.MultiStorey
{
    public partial class SolutionFrameworkServer : SolutionFrameworkService
    {
        ...
        public FunctionResult<Solution> MakeCodeForMultiStoreySolution(List<string> tables, SolutionTemplate codeTemplatee, string databaseConnection, string databaseName, string savePath)
        {
            ...
            MakeCodeForViewService(tables, serviceTemplate, databaseConnection, databaseName, savePath);
            return result;
        }
        ...
        private StringBuilder CreateViewServiceForJava(DatabaseTable t)
        {
            ...
            code.Append(String.Format("public TransactionResult<GridPager<{0}>> GetPager(int page, int size) {{  AssertHelper.AssertRangInt(page,1,Integer.MAX_VALUE); AssertHelper.AssertRangInt(id,size,Integer.MAX_VALUE);TransactionResult<GridPager<{0}>> result = new TransactionResult<GridPager<{0}>>();GridPagerPamater<{0}SearchPamater> pamater = new GridPagerPamater<>();{0}SearchPamater sp = new {0}SearchPamater();pamater.setSearchPamater(sp);pamater.setCurrent(page);pamater.setPageSize(size);GridPager<{0}> serviceReusult = {1}Service.GetPager(pamater);if (serviceReusult.isActionResult() && serviceReusult.isHavingData()){{result.setData(serviceReusult);}}else{{result.setCode(103);result.setMessage(\"暂无数据\");}}return result;}}", t.Name, ParseName(t.Name)));
            //get
            code.Append(" @RequestMapping(value = \"get\")");
            code.Append(String.Format("public TransactionResult<{0}> Get(int id) {{ AssertHelper.AssertRangInt(id,1,Integer.MAX_VALUE); TransactionResult <{0}> result = new TransactionResult<{0}> ();FunctionResult <{0}> serviceResult = {1}Service.Get(id);if (serviceResult.isActionResult() && serviceResult.isHavingData()){{result.setData(serviceResult.getData());} }else{{result.setCode(103);result.setMessage(\"暂无数据\");} }return result;}}", t.Name, ParseName(t.Name)));
            //list
            code.Append("@RequestMapping(value = \"list\")");
            code.Append(String.Format("public TransactionResult<List<{0}>> GetList() {{TransactionResult <List<{0}>> result = new TransactionResult<List<{0}>> ();FunctionListResult<{0}> serviceResult = {1}Service.GetList(new {0}SearchPamater());if (serviceResult.isActionResult() && serviceResult.isHavingData()){{result.setData(serviceResult.getData());} }else{{result.setCode(103);result.setMessage(\"暂无数据\");} }return result;}}", t.Name, ParseName(t.Name)));
            //add
            code.Append("@RequestMapping(value = \"add\")");
            code.Append(String.Format("public TransactionResult<{0}> Add({0} info) {{{2} TransactionResult<{0}> result = new TransactionResult<{0}>();FunctionResult<{0}> serviceResult = {1}Service.Create(info);if (serviceResult.isActionResult() && serviceResult.isHavingData()){{result.setData(serviceResult.getData());} }else{{result.setCode(103);result.setMessage(\"暂无数据\");} }return result;}}", t.Name, ParseName(t.Name), CreateAssertCodeForJava(t)));
            //edit
            code.Append("@RequestMapping(value = \"edit\")");
            code.Append(String.Format("public TransactionResult<Boolean> Edit({0} info) {{{2} TransactionResult <Boolean> result = new TransactionResult<Boolean>();FunctionResult <Boolean> serviceResult = {1}Service.UpdateByID(info);if (serviceResult.isActionResult()){{result.setData(serviceResult.getData());} }else{{result.setCode(103);result.setMessage(\"暂无数据\");} }return result;}}", t.Name, ParseName(t.Name), CreateAssertCodeForJava(t)));
            //del
            code.Append("@RequestMapping(value = \"del\")");
            code.Append(String.Format("public TransactionResult<Boolean> Delete(int id) {{AssertHelper.AssertRangInt(id,1,Integer.MAX_VALUE);TransactionResult <Boolean> result = new TransactionResult<Boolean>(); List<Integer> ids=new ArrayList<>();ids.add(id); FunctionResult <Boolean> serviceResult = {1}Service.DeleteByID(ids);if (serviceResult.isActionResult()){{result.setData(serviceResult.getData());} }else{{result.setCode(103);result.setMessage(\"暂无数据\");} }return result;}}", t.Name, ParseName(t.Name)));
            code.Append("}");
            return code;
        }
    }
